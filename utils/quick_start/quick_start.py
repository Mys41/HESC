    ...
    if if_force and os.path.exists(output_path) and option != 2:
        if not if_force:
            global_logger.log_text(
                f"The output path {output_path} already exists. The program has been terminated because the force mode is not used. Please check and replace it with a non-existent path.",
                desc='error')
            assert 0
        else:
            shutil.rmtree(output_path)
            global_logger.log_text(
                f"The output path {output_path} already exists, please check and replace it with a non-existent path.",
                desc='warning')
